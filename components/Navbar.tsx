...
const Navbar: React.FC = () => {
  ...
  const logoTextMain = isTransparent
    ? 'text-white'
    : 'text-slate-900 dark:text-white';
  const logoTextAccent = isTransparent
    ? 'text-indigo-400'
    : 'text-indigo-600 dark:text-indigo-400';
  const logoSubText = isTransparent
    ? 'text-slate-300'
  ...
  const mobileNavClasses = ({ isActive }: { isActive: boolean }) =>
    `block px-3 py-2 rounded-md text-base font-medium ${
      isActive
        ...
        : 'text-slate-700 dark:text-slate-300 hover:text-indigo-600 hover:bg-slate-50 dark:hover:bg-slate-800'
    }`;
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='flex justify-between items-center'>
          ...
          <NavLink to='/' className='flex items-center space-x-3 group'>
            {logoUrl && logoLoaded ? (
              <img
                src={logoUrl}
                alt={agencyName}
                className='h-10 w-10 object-contain drop-shadow-lg transition-transform duration-300 group-hover:scale-105'
              ...
              <div className='relative'>
                ...
                <div className='relative p-2.5 bg-gradient-to-tr from-violet-600 to-indigo-600 rounded-xl shadow-lg shadow-indigo-500/20 group-hover:scale-105 transition-transform duration-300 ring-1 ring-white/10'>
                  <Layers className='h-6 h-6 text-white' />
                </div>
              </div>
            )}
            ...
            <div className='flex flex-col'>
              <span
                className={`text-xl font-extrabold tracking-tight leading-none ${logoTextMain} transition-colors duration-300`}
              >
                {mainTextFirst}
                <span
                  className={`${logoTextAccent} transition-colors duration-300`}
                >
                  {mainTextSecond}
                </span>
              </span>
              <span
                className={`text-[0.65rem] font-bold tracking-[0.2em] uppercase ${logoSubText} transition-colors duration-300 mt-0.5`}
              >
                {subText}
              </span>
            </div>
          ...
          <div className='hidden md:flex space-x-8 items-center'>
            ...
            <NavLink to='/services' className={navClasses}>
              Services
            </NavLink>
            <NavLink to='/portfolio' className={navClasses}>
              Portfolio
            </NavLink>
            <NavLink to='/proposal' className={navClasses}>
              Proposal
            </NavLink>
            <NavLink
              to='/contact'
      ...
      {isOpen && (
        <div className='md:hidden absolute top-full left-0 w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-2xl border-t border-slate-100 dark:border-slate-800 animate-slide-down'>
          <div className='px-4 pt-4 pb-8 space-y-2'>
            <NavLink to='/' className={mobileNavClasses}>
              Home
            </NavLink>
            <NavLink to='/about' className={mobileNavClasses}>
              About
            </NavLink>
            <NavLink to='/services' className={mobileNavClasses}>
              Services
            </NavLink>
            <NavLink to='/portfolio' className={mobileNavClasses}>
              Portfolio
            </NavLink>
            <NavLink to='/proposal' className={mobileNavClasses}>
              Proposal
            </NavLink>
            <NavLink to='/contact' className={mobileNavClasses}>
              Contact Us
            </NavLink>
          </div>
        </div>
